...
class SqlBuilderTest {
    ...
    fun testSqlSortQuery() {
        ...
        val expectedQueryResult =
            SimpleSQLiteQuery("SELECT * FROM Service AS T_FINAL WHERE `name` LIKE '%abc%' OR `employeeNumber` LIKE '%abc%' ").sql
        Assert.assertEquals(expectedQueryResult, actualQueryResult)
    }
    ...
    fun testSqlSortQueryWithRelation() {
        ...
        val actualQueryResult = formQueryBuilder.getQuery("abc").sql
        val expectedQueryResult =
            SimpleSQLiteQuery("SELECT * FROM Table_3 AS T_FINAL WHERE `LastName` LIKE '%abc%' OR EXISTS ( SELECT * FROM RELATED_TABLE AS S2 WHERE T_FINAL.__relation4Key = S2.__KEY AND S2.field_x LIKE '%abc%' ) ").sql
        Assert.assertEquals(expectedQueryResult, actualQueryResult)
    }
    ...
    fun testSqlSortQueryWithCustomFormatNotFound() {
        ...
        val actualQueryResult = formQueryBuilder.getQuery("abc").sql
        val expectedQueryResult =
            SimpleSQLiteQuery("SELECT * FROM Table_3 AS T_FINAL WHERE `LastName` LIKE '%abc%' OR `field_x` LIKE '%abc%' ").sql
        Assert.assertEquals(expectedQueryResult, actualQueryResult)
    }
    ...
    fun testSqlSortQueryWithCustomFormatFound() {
        ...
        val expectedQueryResult =
            SimpleSQLiteQuery("SELECT * FROM Table_3 AS T_FINAL WHERE `LastName` LIKE '%UX%' OR `field_x` LIKE '%UX%' OR field_x == '0' ").sql
        Assert.assertEquals(expectedQueryResult, actualQueryResult)
    }
    ...
    fun testSqlSortQueryWithRelationWithCustomFormatFound() {
        ...
        val actualQueryResult = formQueryBuilder.getQuery("UX").sql
        val expectedQueryResult =
            SimpleSQLiteQuery("SELECT * FROM Table_4 AS T_FINAL WHERE `LastName` LIKE '%UX%' OR EXISTS ( SELECT * FROM RELATED_TABLE AS S2 WHERE T_FINAL.__relationFieldKey = S2.__KEY AND ( S2.field_1 LIKE '%UX%' OR S2.field_1 == '0' ) ) ").sql
        Assert.assertEquals(expectedQueryResult, actualQueryResult)
    }
